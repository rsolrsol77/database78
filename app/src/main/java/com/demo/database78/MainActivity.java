...
//loog in
    ...
    private DocumentReference premiumUserRef;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (user != null) {
            ...
            checkPremiumUserStatus(user.getUid());
    ...
    // ========== نظام التحقق من المستخدم المميز ==========
    private void checkPremiumUserStatus(String userId) {
        FirebaseFirestore db = FirebaseFirestore.getInstance();
        premiumUserRef = db.collection("premium_users").document(userId);

        // مراقبة التغييرات الفورية
        premiumUserRef.addSnapshotListener((snapshot, error) -> {
            if (error != null) {
                Log.e("PremiumCheck", "خطأ في مراقبة الحالة", error);
                return;
            }

            // تحديث حالة المستخدم
            boolean isPremium = snapshot != null && snapshot.exists();
            this.isPremiumUser = isPremium;

            // حفظ الحالة للتطبيق بأكمله
            SharedPreferences prefs = getSharedPreferences("user_prefs", MODE_PRIVATE);
            prefs.edit().putBoolean("is_premium", isPremium).apply();

            Log.d("PremiumCheck", "حالة المستخدم المحدثة: " + (isPremium ? "مميز" : "عادي"));
        });
    }

    // دالة ثابتة للتحقق من حالة المستخدم من أي نشاط
    ...
    protected void onDestroy() {
        super.onDestroy();
        // تنظيف المراقبين عند تدمير النشاط
        if (premiumUserRef != null) {
            premiumUserRef = null;
        }
    }
    // ========== نهاية النظام الجديد ==========
